...
| **结果对比** | 「被搜图 vs 结果图」并排 / 滑动擦除 / 差异叠加三种模式，视角同步 + 属性差异表 |
...
├── src/                       # 渲染进程（Vue 应用）
│   ├── components/            # 侧边栏 / 工具栏 / 虚拟网格 / 预览抽屉 / 对比视图 / 状态栏
...
以图搜图后，工具栏会出现**被搜图**缩略卡片，随时提示当前结果基于哪张图检索。
点击卡片上的**对比**（或悬停结果图右下角的「对比」、预览抽屉内的「与被搜图对比」）
打开对比视图：

| 模式 | 用途 |
| --- | --- |
| **并排** | 左右两图对照，看整体构图与色调差异 |
| **滑动** | 同位置叠放，拖动分割线擦除，看局部细节差异 |
| **差异** | 上层 `difference` 混合，相同区域趋近纯黑，快速定位改动位置 |

- 两张图**共享同一缩放与平移**，滚轮缩放、拖拽平移始终保持相同视角
- 支持**完整/填充**两种适应方式，`←` `→` 或按钮可在结果间连续切换
- 下方给出相似度仪表与**属性差异表**：分辨率、宽高比、体积、格式、修改时间、路径，
  差异项高亮并标注变化量（如 `+42% 像素`、`+1.20 MB`）
- 尺寸与体积完全一致时标记**疑似重复文件**，便于清理冗余图片

> 外部上传/拖入的图片虽未入库，也会读取其文件元信息参与对比。
